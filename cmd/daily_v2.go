...
func newDailyV2Command(registryFactory vaultRegistryFactory, serviceFactory noteServiceFactory, now func() time.Time) *cobra.Command {
	var common commonFlags
	command := newV2Namespace("daily", "Resolve and safely mutate Obsidian Daily Notes", &common)
	if now == nil {
		now = time.Now
	}
	...
	var getDate string
	get := &cobra.Command{Use: "get", Args: v2Args(&common, "daily.get", cobra.NoArgs)}
	get.RunE = func(cmd *cobra.Command, _ []string) error {
		return renderV2(cmd, "daily.get", common.RequestID, func() (any, error) {
	...
	create := &cobra.Command{Use: "create", Args: v2Args(&common, "daily.create", cobra.NoArgs)}
	create.RunE = func(cmd *cobra.Command, _ []string) error {
		return renderV2(cmd, "daily.create", common.RequestID, func() (any, error) {
			service, vault, err := resolveNoteContext(common, registryFactory, serviceFactory)
			if err != nil {
				return nil, err
			}
	...
	append := &cobra.Command{Use: "append", Args: v2Args(&common, "daily.append", cobra.NoArgs)}
	append.RunE = func(cmd *cobra.Command, _ []string) error {
		return renderV2(cmd, "daily.append", common.RequestID, func() (any, error) {
