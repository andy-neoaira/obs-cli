...
func newMetadataV2Command(registryFactory vaultRegistryFactory, serviceFactory noteServiceFactory) *cobra.Command {
	var common commonFlags
	command := newV2Namespace("metadata", "Read and safely update note frontmatter", &common)

	get := &cobra.Command{Use: "get <path>", Args: v2Args(&common, "metadata.get", cobra.ExactArgs(1))}
	get.RunE = func(cmd *cobra.Command, values []string) error {
		return renderV2(cmd, "metadata.get", common.RequestID, func() (any, error) {
	...
	set := &cobra.Command{Use: "set <path>", Args: v2Args(&common, "metadata.set", cobra.ExactArgs(1))}
	set.RunE = func(cmd *cobra.Command, values []string) error {
		return renderV2(cmd, "metadata.set", common.RequestID, func() (any, error) {
