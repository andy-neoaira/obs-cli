...
func TestDailyUsesObsidianConfigTemplateAndRevision(t *testing.T) {
	...
	get := executeTestCommand(t, newDailyCommand(registryFactory, serviceFactory, now), "", "get")
	var getData struct {
	...
	dry := executeTestCommand(t, newDailyCommand(registryFactory, serviceFactory, now), "", "create", "--dry-run")
	if !dry.OK {
	...
	created := executeTestCommand(t, newDailyCommand(registryFactory, serviceFactory, now), "", "create")
	if !created.OK {
	...
	appended := executeTestCommand(t, newDailyCommand(registryFactory, serviceFactory, now), "- shipped",
	...
	withoutTemplate := executeTestCommand(t, newDailyCommand(registryFactory, serviceFactory, now), "", "get")
	if !withoutTemplate.OK {
		t.Fatalf("existing daily depends on deleted template: %#v", withoutTemplate)
	}
	conflict, _, err := executeTestCommandResult(
		newDailyCommand(registryFactory, serviceFactory, now), "again",
...
func TestMetadataPreservesUnknownFieldsAndBody(t *testing.T) {
	registryFactory, serviceFactory, vaultRoot := noteCommandDependencies(t)
	service, err := serviceFactory(vaultRoot)
	if err != nil {
		t.Fatal(err)
	...
	command := func() *cobra.Command { return newMetadataCommand(registryFactory, serviceFactory) }

	dry := executeTestCommand(t, command(), "", "set", "Projects/Demo",
	...
	applied := executeTestCommand(t, command(), "", "set", "Projects/Demo",
	...
	conflict, _, err := executeTestCommandResult(command(), "",
...
func TestDailyDoesNotOverwriteDailyCreatedByAnotherEntry(t *testing.T) {
	registryFactory, serviceFactory, vaultRoot := noteCommandDependencies(t)
	...
	conflict, _, err := executeTestCommandResult(
		newDailyCommand(registryFactory, serviceFactory, now), "", "create",
	)
...
func TestDailyRejectsMalformedConfigWithoutWriting(t *testing.T) {
	registryFactory, serviceFactory, vaultRoot := noteCommandDependencies(t)
	...
	for _, operation := range []string{"get", "create"} {
		response, _, err := executeTestCommandResult(
			newDailyCommand(registryFactory, serviceFactory, now), "", operation,
		)
...
func executeTestCommand(t *testing.T, command *cobra.Command, stdin string, args ...string) noteTestEnvelope {
	t.Helper()
	response, output, err := executeTestCommandResult(command, stdin, args...)
	if err != nil {
...
func executeTestCommandResult(command *cobra.Command, stdin string, args ...string) (noteTestEnvelope, string, error) {
