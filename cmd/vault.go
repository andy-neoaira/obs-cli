...
func newVaultCommand(factory vaultRegistryFactory, discover vaultDiscoverFunc) *cobra.Command {
	var common commonFlags

	command := &cobra.Command{
		Use:   "vault",
		Short: "Manage the obs-cli vault registry",
	...
	args := func(operation string, validate cobra.PositionalArgs) cobra.PositionalArgs {
		return func(cmd *cobra.Command, values []string) error {
			if err := validate(cmd, values); err != nil {
				...
				return renderEnvelope(cmd, operation, common.RequestID, func() (any, error) {
					return nil, domain
				})
			}
			return nil
	...
	command.AddCommand(&cobra.Command{
		Use:   "list",
		Short: "List vaults registered in obs-cli",
	...
	add := &cobra.Command{
		Use:   "add <path>",
		Short: "Register a directory in obs-cli",
	...
	remove := &cobra.Command{
		...
		Short: "Remove a vault from obs-cli without deleting files",
	...
	setDefault := &cobra.Command{
		...
		Short: "Set the default obs-cli vault",
