...
func currentCapabilities() capabilitiesData {
	return capabilitiesData{
		...
		Operations: []capabilityOperation{
			...
			{Name: "doctor.audit", Version: 1, CommonFlags: []string{"output", "request-id"}},
			...
			{Name: "note.move", Version: 1, Mutating: true, CommonFlags: []string{"dry-run", "if-match", "if-plan-hash", "output", "request-id", "vault"}},
			...
			{Name: "update.apply", Version: 1, Mutating: true, CommonFlags: []string{"dry-run", "output", "request-id"}},
			{Name: "update.check", Version: 1, CommonFlags: []string{"output", "request-id"}},
...
func newCapabilitiesCommand() *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "capabilities",
		Short: "Describe the machine-readable obs-cli capability surface",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			resolved, err := protocol.ResolveRequestID(common.RequestID)
			if err != nil {
				resolved, _ = protocol.ResolveRequestID("")
				return renderEnvelope(cmd, "capabilities.get", resolved, func() (any, error) {
					return nil, err
				})
			}
			...
			if common.Output != "json" {
				...
				return renderEnvelope(cmd, "capabilities.get", common.RequestID, func() (any, error) {
					return nil, domain
				})
			}
