...
func newLinkCommand(registryFactory vaultRegistryFactory, serviceFactory noteServiceFactory) *cobra.Command {
	var common commonFlags
	command := newNamespace("link", "Inspect Obsidian links without modifying notes", &common)
	var scope string
	var maxFiles int
	backlinks := &cobra.Command{
		Use:  "backlinks <path>",
		Args: namespaceArgs(&common, "link.backlinks", cobra.ExactArgs(1)),
	}
	backlinks.RunE = func(cmd *cobra.Command, values []string) error {
		return renderEnvelope(cmd, "link.backlinks", common.RequestID, func() (any, error) {
