...
func newSearchV2Command(registryFactory vaultRegistryFactory, serviceFactory noteServiceFactory) *cobra.Command {
	var common commonFlags
	command := newV2Namespace("search", "Search Markdown notes with bounded revision evidence", &common)
	var scope string
	var page, pageSize, maxFiles int
	content := &cobra.Command{
		Use:  "content <query>",
		Args: v2Args(&common, "search.content", cobra.ExactArgs(1)),
	}
	content.RunE = func(cmd *cobra.Command, values []string) error {
		return renderV2(cmd, "search.content", common.RequestID, func() (any, error) {
