...
- [x] [P1-T07 Move 与链接重写事务](./P1-T07-move-link-transaction.md)
- [x] [P1-T08 V2 命令树、质量门禁与发布迁移](./P1-T08-v2-release.md)

推荐顺序：P1-T01/P1-T02/P1-T04 → P1-T03 → P1-T05 → P1-T06 → P1-T07 → P1-T08。

## 阶段完成标准

- [x] 所有 Agent 核心命令支持 JSON。
- [x] 所有文件写入使用统一原子存储层。
- [x] 所有更新型操作支持 revision 前置条件。
- [x] 路径和符号链接不能逃逸 Vault。
- [x] 批量或多文件修改不会静默部分成功。
- [x] Human-first 隐式行为已删除或移出核心命令。
- [x] 单元、竞态、覆盖率和发布检查通过。
...
GOCACHE=/private/tmp/obs-cli-gocache make release-check
```
