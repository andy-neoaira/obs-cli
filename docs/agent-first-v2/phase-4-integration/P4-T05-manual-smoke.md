# P4-T05 人工 Obsidian 冒烟清单

> 本清单必须使用专用测试 Vault。禁止使用个人 Vault。自动化脚本不能替代
> Obsidian 桌面端和移动端的真实行为验证。

## 测试信息

- 状态：`待人工执行`
- 测试人：`待填写`
- 桌面端版本：`待填写`
- 移动端平台与版本：`待填写`
- 同步方式与版本：`待填写`
- 执行日期：`待填写`
- 测试 Vault：`专用临时/测试 Vault，路径待填写`

## 前置门禁

- [x] `./scripts/run-three-client-e2e.sh` 输出 `6/6`。
- [ ] 测试 Vault 已备份或可随时丢弃。
- [ ] 桌面端、移动端、CLI 和 Neovim 指向同一份同步数据。
- [ ] 已记录同步服务的冲突副本、延迟与离线策略。

## 桌面端

- [ ] 在 Obsidian 创建含 Frontmatter 与 Wikilink 的笔记。
- [ ] Agent 通过 CLI 搜索并以 revision 前置条件更新该笔记。
- [ ] Neovim 打开后内容、Frontmatter 和链接一致。
- [ ] Neovim 新建笔记后，Obsidian 文件列表和图谱可发现它。
- [ ] CLI 移动笔记并重写链接后，Obsidian 不出现新增坏链接。
- [ ] Obsidian 外部修改发生在 Agent plan/apply 之间时，apply 返回冲突且不覆盖。
- [ ] 三个入口打开同一天的 Daily Note，路径和模板结果一致。

## 移动端与同步

- [ ] 移动端能打开桌面端/Agent/Neovim 创建的笔记。
- [ ] 移动端编辑同步回桌面后，CLI 读取到新的 revision。
- [ ] 离线编辑与 Agent 并发更新时，不发生无提示的数据覆盖。
- [ ] 若同步服务生成冲突副本，已记录命名、位置和恢复步骤。
- [ ] 已测量并记录典型同步延迟；测试期间不把“尚未同步”误判为文件丢失。
- [ ] 已确认移动端插件限制不会改变 Vault 文件格式或 Daily Note 路径。

## 结果记录

| 项目 | 结果 | 证据/备注 |
|---|---|---|
| 桌面端闭环 | 待测 | |
| 移动端读取与回写 | 待测 | |
| 并发冲突保护 | 待测 | |
| Daily Note 一致性 | 待测 | |
| 移动与链接完整性 | 待测 | |
| 同步限制 | 待记录 | |

只有所有必测项通过，且限制与恢复步骤已记录后，才能把 P4-T05 标记为
`已完成`。
