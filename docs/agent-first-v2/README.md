...
1. 默认按 P0 → P1/P2 → P3 → P4 执行；P5 在审计确认后执行。
2. P1 与 P2 可在 P0 完成后并行开发。
3. P5 可与 P4 的移动端人工验证并行，但不得借审计修复改变已冻结的 V2 响应结构。
4. 一个子任务应对应一个独立 PR 或一组可独立回滚的提交。
5. 不因任务实施而扩大范围；新需求先新增任务文档并标注依赖。
6. 修改共同语义时，必须同步更新规范、共享 fixture 和两个实现的测试。
7. 任务只有在交付物齐全、验收项全部勾选、验证命令通过后才能标记完成。
