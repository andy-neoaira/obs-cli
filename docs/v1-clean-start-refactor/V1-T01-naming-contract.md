...
- [x] `ConfigPath` 作为唯一配置路径函数名。
- [x] 删除 `vault migrate` 和旧 `preferences.json` 导入。
- [x] 删除五个 `_v2` feature flag，不提供替代别名。
- [x] 删除 `docs/agent-first-v2`，不在仓库内归档。
- [x] 删除 `docs/migration/V1_TO_V2.md`，不提供新旧产品迁移承诺。
- [x] 发布候选版本固定为 `v1.0.0-rc.1`。

## 验收标准

- [x] 映射表不存在“稍后决定”或多个候选目标名。
- [x] 两个仓库对协议、产品版本和保留契约的名称一致。
- [x] 所有项目自有 V2 名称都有明确处置。
- [x] 第三方 `/v2` 有明确 allowlist 策略。
- [x] 配置硬切换和本地开发配置处理方式已写明。
- [x] 后续任务无需重新讨论命名。
...
- 完成日期：`2026-07-28`
- obs-cli 提交：`592b0e9`
- miniobsidian.nvim 基线：`184662ddddadf6fe887d8897e5874c37779757c2`
- 最终 Capability 决策：`删除 5 个历史 _v2 flag，不提供别名`
- 历史文档决策：`删除当前仓库中的旧执行文档，历史由 Git 保留`
