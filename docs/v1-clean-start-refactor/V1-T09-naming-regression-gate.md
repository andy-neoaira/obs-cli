...
- [x] 第三方 `/v2` 不误报。
- [x] allowlist 最小且有注释。
- [x] 门禁进入本地 release-check 和 GitHub CI。
- [x] 错误输出能直接定位文件和模式。
- [x] 门禁不依赖网络。
...
- 完成日期：`2026-07-28`
- 提交：`46767b7`
- 禁止模式数量：`17 个内容模式 + versioned filename 检查`
- allowlist：`第三方 go.mod/go.sum/vendor major version 不纳入项目命名扫描`
- 失败注入证据：`scripts/naming-check.sh --self-test 通过`
