...
- [x] 唯一默认配置路径为 `obs-cli/config.json`。
- [x] 新配置固定写入 `"version": 1`。
- [x] `version != 1` 明确拒绝。
- [x] 只有旧文件时不会自动读取或转换。
- [x] 不存在旧配置常量、路径函数或 alias。
- [x] 原子写入、锁、权限、并发更新测试通过。
- [x] dry-run 保持零配置副作用。
...
- 完成日期：`2026-07-28`
- 提交：`bb2a823`
- 配置负向测试：`version: 2 明确拒绝；只有 config-v2.json 时不读取、不迁移`
- legacy import 删除证据：`vault migrate、preferences.json importer 与旧 GUI open type 已删除`
