...
- 状态：已接受
- 决策日期：2026-07-24
- 决策范围：`obs-cli` V1、场景化 Skills、`miniobsidian.nvim`、Obsidian Vault
...
产品边界必须保持冻结，避免两个项目重复实现复杂批处理逻辑，或形成 `Neovim → CLI → Obsidian` 的错误依赖链。
...
```text
       ...
       ┌─────────────────┼─────────────────┐
       │                 │                 │
 Obsidian App       obs-cli V1      miniobsidian.nvim
...
### 2. obs-cli V1 是 Agent-first 执行器

`obs-cli` V1 的首要消费者是 AI Agent、Skill、脚本和自动化系统。核心能力必须：
...
- `obs-cli` 可以只读发现 Obsidian 配置，不直接写入官方全局配置。
...
| obs-cli V1 | 机器协议、安全原子操作、批处理计划、冲突与错误模型 | TTY picker、内置编辑器体验、Neovim UI |
...
未采用。现有 Go 项目的 Vault、Frontmatter、链接和测试基础可以复用，在当前仓库内维护 Agent-first 实现的风险和成本更低。
...
- `obs-cli` V1 通过显式版本协议和配置拒绝隐式兼容路径。
...
Vault 共同语义由 `vault-contract/v1` fixture 验证；JSON、Capability、revision 和原子
写入由当前 Schema 与发布门禁验证；插件独立性和三入口闭环由
`miniobsidian.nvim` CI 与三入口 E2E 验证。
