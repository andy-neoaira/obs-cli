...
// ConfigPath 返回 Agent-first CLI 自有配置目录和配置文件的完整路径。
// 配置目录遵循操作系统标准：
//   - Linux/Mac: ~/.config/obs-cli/
//   - Windows: %APPDATA%\obs-cli\
//
// 该文件与 Obsidian 官方 obsidian.json 完全分离。
func ConfigPath() (cliConfigDir string, cliConfigFile string, err error) {
	...
	cliConfigFile = filepath.Join(cliConfigDir, ConfigFile)
